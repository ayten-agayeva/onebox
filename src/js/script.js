$(document).ready(() => {
    ...
    $('.custom-select-search').select2({dropdownCssClass: 'select-search'});
        ...
        .call(sliders, (slider) => {
            slider
                ...
                .addEventListener('input', (event) => {
                    slider
                        .querySelector('input.range-slider__value')
            ...
            slider
                ...
                .addEventListener('keyup', (event) => {
                    const val = $
                        ...
                        .replace(/[^0-9.]/g, '');
                    slider
                        .querySelector('input.range-slider__range')
                        .value = val == ""
                            ? 0
                            : val;
                    slider
    ...
    function applyFill(slider) {
        ...
        var bg = `linear-gradient(90deg, ${settings.fill} ${percentage}%, ${settings.background} ${percentage +
                0.1}%)`;
    ...
    $('body').on('click', '.tabs[data-target] > *:not(.active)', function (e) {
        ...
        if (tab_parent.length > 0 && target) {
            ...
            const tab = $(this),
                ...
                index = tab.data('index') != undefined
                    ? tab.data('index')
                    : tabs.index(tab),
                content = $(target).find('>*');
    ...
    $('.datepicker_h').datetimepicker(
        {format: 'DD.MM.YYYY, HH:mm', autoclose: true}
    );
    ...
    if ($('.fine-uploader-gallery').length > 0) {
        ...
        $('.fine-uploader-gallery').fineUploader({
            debug: true, button: document.getElementsByClassName('add-invoice-file')[0],
            autoUpload: false,
            ...
            callbacks: {
                ...
                onStatusChange: function (id, oldStatus, newStatus) {
                    ...
                    if ($('.img-list').length) {
                        $('.invoys_upload_box')
                            .removeClass('invoys_upload_box')
                            .addClass('invoys_upload_selected_file')
                    } else {
                        $('.invoys_upload_selected_file')
                            .removeClass(
                                'invoys_upload_selected_file with-error'
                            )
                            .addClass('invoys_upload_box')
                    }
                }
            }
        });
